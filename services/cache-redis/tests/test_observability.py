"""
Stage 8 — sidecar telemetry validation for cache-redis.
...
COLLECTOR_CONTAINER = "openframe-cache-redis-collector"
SPANS_FILE          = "/var/otel/spans.jsonl"
EXPECTED_SERVICE    = "cache-redis"
POLL_TIMEOUT_S      = 30
POLL_INTERVAL_S     = 0.5
...
def _create_session() -> httpx.Response:
    session_id = f"stage8-{uuid.uuid4().hex[:8]}"
    with httpx.Client(base_url=SERVICE_URL, timeout=15.0) as client:
        return client.post(
            "/sessions",
            json={"id": session_id, "user_id": "test-user-stage8"},
        )
