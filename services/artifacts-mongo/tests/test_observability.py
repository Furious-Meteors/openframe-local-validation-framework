"""
Stage 8 — sidecar telemetry validation for artifacts-mongo.
...
COLLECTOR_CONTAINER = "openframe-artifacts-mongo-collector"
SPANS_FILE          = "/var/otel/spans.jsonl"
EXPECTED_SERVICE    = "artifacts-mongo"
POLL_TIMEOUT_S      = 30
POLL_INTERVAL_S     = 0.5
...
def _post_artifact() -> httpx.Response:
    ...
    with httpx.Client(base_url=SERVICE_URL, timeout=15.0) as client:
        return client.post(
            "/artifacts",
            json={"id": artifact_id, "title": "Stage8 Telemetry Paper",
                  "source": "test-suite", "tags": ["stage8"]},
        )
